...
export async function getAppointments({
  ...
  const orgId = await getCurrentOrganizationFromCookie();
  const organizationId = orgId?.id || (user?.organization ? user.organization.toString() : null);
  const organizationObjectId = organizationId
  ...
  if (user.role !== "SUPERADMIN") {
    ...
    andConditions.push({ organization: organizationObjectId });
  }
...
export async function updateAppointment(appointmentId: string, data: AppointmentUpdateData) {
  try {
    ...
    if (user.role === 'DOCTOR') {
      query.$or = [{ doctor: user._id }, { createdBy: user._id }];
    }
    ...
    await Appointment.findOneAndUpdate(query, updateData);
...
export async function deleteAppointment(appointmentId: string) {
  try {
    ...
    const appointment = await Appointment.findOne(query).select('_id patient doctor').lean();
    if (!appointment) {
